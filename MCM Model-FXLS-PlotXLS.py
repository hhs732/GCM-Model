...
SeasonalSumVar = np.empty((SizeSVar[1], 4, SizeSVar[0]))
...
def Write2XLS(ProjectedData,Sheetname,SizeProjData,Time,XLSFileName,Plot):
    Output = xlsxwriter.Workbook()
    for v in range (SizeProjData[2]):
        Sheet = Output.add_worksheet(Sheetname[v])
        Sheet.write(0, 0, "100PY/Time")
        for p in range (SizeProjData[1]): 
            Sheet.write(0, p+1, Time[p])
        for q in range (SizeProjData[0]):
...
SheetnameANN = ['ANNTemp', 'ANNPrecip', 'ANNEvap', 'ANNSnow', 'ANNRainDay', 'ANNDayL0', 'ANNDayH40']
Year = np.array(["Yearly"])
Write2XLS(ANNClimVar,SheetnameANN,SizeANNVar,Year,XLSNameANN,1)

#workbook = xlwt.Workbook('chart_line.xlsx')
