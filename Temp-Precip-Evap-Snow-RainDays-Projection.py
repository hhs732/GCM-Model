...
WidFcnJet = Const[44,1]	
LatFcnJet = Const[45,1]	
...
#Precip/RD = Pobs/RDobs
FcnJetmRD = np.exp(-0.5*np.power([(LatFcnJet - X)/WidFcnJet for X in Jet120Wmonth],2))      
...
#FcnJetyRD = np.exp(-0.5*np.power([(LatFcnJet - X)/WidFcnJet for X in Jet120Wyear],2))      
...
for p in range (len(HPrecip.T)):     
    for q in range (len(HPrecip)):
        ...
        HRainDay[q,p] = (CRD + (XCPrecRD * HPrecip[q,p]) + (XCJetRD * Jet120Wyear[q,p]) + (XCFcnJetRD * FcnJetyRD) + (XCITCRD * ITCyear[q,p]) +(XCFcnITCRD * FcnITCyRD)) * (RDobs[p]/RDrawCalNow[p])
